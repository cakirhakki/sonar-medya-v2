...
(function ($) {
	...
	////////////////////////////////////////////////////
	// PreLoader – sağlam sürüm
  (function () {
    const $loader = $('#loading');
    const bar = document.getElementById('tp-loading-line');

    const done = () => {
      $loader.addClass('loaded').fadeOut(400);
      document.body.style.overflow = '';
    };

    // 3 sn fail-safe (assets sorununda bile kapanır)
    const failSafe = setTimeout(done, 3000);

    // Sadece ilk ekranda yüklenen img'ler (lazy olanları sayma)
    const imgs = Array.from(document.images)
      .filter(img => img.getAttribute('loading') !== 'lazy');

    if (!imgs.length) { clearTimeout(failSafe); return done(); }

    let loaded = 0, tot = imgs.length;

    const tick = () => {
      loaded++;
      if (bar) bar.style.width = Math.floor((loaded / tot) * 100) + '%';
      if (loaded >= tot) { clearTimeout(failSafe); done(); }
    };

    imgs.forEach(img => {
      if (img.complete) return tick();                 // cache'den gelmiş olabilir
      img.addEventListener('load',  tick, { once: true });
      img.addEventListener('error', tick, { once: true });
    });

    // Ek garanti: sayfa tamamen yüklendiyse yine kapat
    window.addEventListener('load', () => { clearTimeout(failSafe); done(); });
  })();
