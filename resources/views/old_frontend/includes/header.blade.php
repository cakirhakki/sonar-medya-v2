<header>
                        ...
                        <div class="col-xxl-3 col-xl-2 col-lg-2 col-md-4 col-sm-5 col-8">
                            <div class="logo">
                                <a href="{{ route('site.home') }}">
                                    <img class="site-logo"
                                        src="{{ $site?->logo_url ?? asset('site/images/sonar-logo-renkli.png') }}"
                                        alt="{{ $site?->site_title ?? config('app.name', 'Sonar Medya') }}">
                                </a>
                            </div>
                        </div>
    ...
    @php
        /** @var \Illuminate\Support\Collection|\App\Models\ServiceCategory[] $cats */
        $cats      = $menuServiceCategories ?? collect();
        $serviceMap = $menuCategoryServices ?? [];
        $limit     = 3;
    ...
    @if ($cols->isNotEmpty())
        <ul class="mega-menu">
            @foreach ($cols as $cat)
                @php
                    // Menü için filtrelenmiş hizmet listesi: önce composer’dan, yoksa model aksesörü
                    $services = collect($serviceMap[$cat->id] ?? $cat->menu_service_list);
                @endphp
                ...
                <li>
                    ...
                    <ul>
                        @forelse ($services as $svc)
                            <li>
                                <a href="{{ route('frontend.services.show', $svc->slug) }}">
                                    {{ $svc->name }}
                                </a>
                            </li>
                        @empty
                        ...
                        <div class="col-xxl-2 col-xl-2 col-lg-3 col-md-8 col-sm-7 col-4">
                            <div class="header__bottom-right-6 d-flex justify-content-end align-items-center pl-30">
                                <div class="header__btn-6 d-none d-sm-block">
                                    <a href="https://portal.sonarmedya.com/" target="_blank"
