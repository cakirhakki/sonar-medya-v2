    ...
    public function index()
    {
        ...
        $homeFeed = Post::query()
            ...
            ->select([
                ...
                'author_id','primary_post_category_id',
            ])
            ->latest('published_at')
            ->take(6)
            ->get();

        // 2) “Öne çıkan hizmet kategorileri” için 4 kart
        ...
        // Tema renk sınıfları ve animasyon gecikmeleri
        $cardClasses = ['', 'has-green', 'has-pink', 'has-purple'];
        $delays      = ['.3s', '.5s', '.7s', '.9s'];

        // Kart ikonları (temadaki statik SVG’ler)
        $icons = [
            // 1) E-Ticaret
            <<<SVG
...
SVG,
            // 3) SEO
            <<<SVG
<svg width="43" height="42" viewBox="0 0 43 42" fill="none" aria-hidden="true">
...
<svg width="42" height="42" viewBox="0 0 42 42" fill="none" aria-hidden="true">
  ...
  <path d="M41.0001 29C41.0001 35.62 35.6201 41 29 41C25.92 41 23.12 39.84 21 37.92C23.46 35.74 25 32.54 25 29C25 27.44 24.7 25.94 24.16 24.58C27.82 23.58 30.78 20.8999 32.16 17.4199C37.2401 18.7999 41.0001 23.4799 41.0001 29Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
SVG,
        ];

        $featureCards = $cats->values()->map(function ($cat, $i) use ($icons, $cardClasses, $delays) {
            return [
                'title'      => $cat->name,
                'desc'       => $cat->description ?? '',
                'url'        => route('frontend.services.category', $cat->slug),
                'icon_svg'   => $icons[$i] ?? $icons[0],
                'card_class' => $cardClasses[$i] ?? '',
                'wow_delay'  => $delays[$i] ?? '.3s',
            ];
        });

        return view('frontend.pages.index', compact('homeFeed', 'featureCards'));
    }
}
